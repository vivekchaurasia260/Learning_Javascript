...
function sleepIn(weeday, vacation)
{
    return (!weeday || vacation);
}

isweekDay = sleepIn(true, false);
...
function makeBricks(oneInch, fiveInch, goal)
{   
    // fiveInch *= 5;
    // remBricks = goal%fiveInch;
    // if(remBricks <= oneInch)
    //     return true;
    ...
    return goal%5 >= 0 && goal%5 - oneInch <= 0 && oneInch + 5*fiveInch >= goal; 
}

console.log(makeBricks(3, 1, 9));
console.log(makeBricks(3, 2, 10));
console.log(makeBricks(2, 3, 17));
